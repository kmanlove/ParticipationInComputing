...
#  institution.list <- institution.list2 <- author.institutions <- vector("list", length = dim(data.frame.in)[1])
  author.list <- author.lastonly <- author.firstonly <- author.position <- tot.authors <- vector("list", length = dim(data.frame.in)[1])
  pub.year <- pubyear <- source.2 <- source.1 <- vector("list", length = dim(data.frame.in)[1])
  author.list.out <- vector("list", length = dim(data.frame.in)[1])
  ...
  { # loop starts at 2 because paper 1 is authored by Anonymous and there's no info in the author fields
    ...
    author.list[[i]] <- strsplit(x = as.character(data.frame.in[i, ]$AF), split = ";")[[1]]
    pub.year[[i]] <- data.frame.in[i, ]$PY[1]
    source.1[[i]] <- data.frame.in[i, ]$SO[1]
    for(j in 1:length(author.list[[i]]))
    {
      ...
      author.lastonly[[i]][j] <- strsplit(x = (author.list[[i]][j]), split = ",", fixed = T)[[1]][1]
      author.firstonly[[i]][j] <- trim(strsplit(x = strsplit(x = (author.list[[i]][j]), split = ",", fixed = T)[[1]][2], split = ". ")[[1]][1])
      author.position[[i]][j] <- j
      tot.authors[[i]][j] <- length(author.list[[i]])
      pubyear[[i]][j] <- pub.year[[i]]
      source.2[[i]][j] <- source.1[[i]]
      } # end j
...
#       )
#     } # END else
      author.list.out[[i]] <- cbind(author.list[[i]], author.lastonly[[i]], author.firstonly[[i]], author.position[[i]], tot.authors[[i]], pub.year[[i]], source.2[[i]])
   } # END i
   
  # unlist author.list.out and store as one large dataframe
  author.full.frame <- as.data.frame(do.call("rbind", author.list.out))
  names(author.full.frame) <- c("FullName", "Last", "First", "Position", "TotAuthors", "PubYear", "Source")
